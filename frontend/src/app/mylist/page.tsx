...
export default function MyListPage() {
  ...
  // Extract city from address helper
  const extractCityFromAddress = (address: string): string => {
    // Match pattern: "XXXX City," or "City,"
    const match = address.match(/\d{4}\s+([^,]+),|^([^,]+),/);
    if (match) {
      return (match[1] || match[2] || '').trim();
    }
    // Fallback: try to extract first word after postal code
    const fallbackMatch = address.match(/\d{4}\s+(\w+)/);
    if (fallbackMatch) {
  ...
  const filteredPlaces = useMemo(() => {
    ...
    if (city) {
      filtered = filtered.filter((place) => {
        ...
        return placeCity.toLowerCase().includes(city.toLowerCase());
      });
    }
